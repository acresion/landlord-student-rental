...
public class TenantDAOImplementation implements TenantDAO{
	private static final String GET_ALL_TENANTS_SQL = 
			"SELECT "
			+ "tenantId"
			+ ", firstName"
			+ ", lastName"
	...
	private static final String UPDATE_TENANT_SQL = "update Tenant set firstName = ?"
			+ ", lastName = ?"
			+ ", addressBeforeMovein = ?"
			+ ", cityBeforeMovein = ?"
			+ ", stateBeforeMovein = ?"
	...
	public List<Tenant> getAllTenants(Connection con) throws SQLException {
		...
		try(PreparedStatement p = con.prepareStatement(GET_ALL_TENANTS_SQL)){
	   		...
	   		while(rs.next()) {
	   			...
	   			Tenant newTenant = new Tenant.TenantBuilder(tenantId).firstName(firstName).lastName(lastName).addressBeforeMovein(addressBeforeMovein).cityBeforeMovein(cityBeforeMovein).stateBeforeMovein(stateBeforeMovein).countryBeforeMovein(countryBeforeMovein).yearOfBirth(yearOfBirth).monthOfBirth(monthOfBirth).dayOfBirth(dayOfBirth).room(room).floor(floor).landlordId(landlordId).build();
